...
> ## ⚠️ 重要变更（v1.3 评审修复版）
>
> - **注册中心权限已改为 `signature`**：`ipc-registry-app` 与所有业务应用必须用**同一签名证书**构建，否则无法绑定注册中心（`normal` 权限等于对任意应用开放）。
> - **请求超时真实生效**：对端不响应时，`get`/`set`/`call` 会在 `requestTimeoutMs`（默认 3000ms）后回调 `TIMEOUT`，不再永久挂起。
> - **订阅丢包可感知**：队列溢出时订阅方会收到 `SubscriptionGap`（错误码 `EVENT_GAP`），不再静默丢帧。
> - **限频/死区有到期补发**：`NotificationPolicy` 抑制的最后一个合格变化会在窗口到期后补发，不会永久丢失。
> - **服务端按契约校验**：未知能力、只读属性写入、不可读属性读取、类型不符、越界/NaN 都会返回明确错误码（`UNKNOWN_CAPABILITY`/`READ_ONLY`/`CAPABILITY_NOT_SUPPORTED`/`TYPE_MISMATCH`/`INVALID_ARGUMENT`）。
> - **void/Unit 命令**：用 `CommandKey.createUnit(id)` / `CommandKey.stringToUnit(id)` 声明；命令声明了返回类型却返回 `null` 会显式报错（不再误报 `TYPE_MISMATCH`）。
> - **ACK 窗口 v1 = 0**（未启用 ACK 流控）；`CommandKey.retryPolicy` 目前**未实现自动重试**，声明后会打 WARN，调用方自行处理重试。
> - **服务关闭即真正下线**：`publisher.close()` 后不再接受新会话，也不会在注册中心重连后「复活」。
> - 新增 `CarIpcConfig` 配置：`strictPermissionMode`、`aclProvider`、`debugLogging`。
>
> 详细修复清单见 [`docs/CODE_REVIEW_FINDINGS.md`](docs/CODE_REVIEW_FINDINGS.md)。
